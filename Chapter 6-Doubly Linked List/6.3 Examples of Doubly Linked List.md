...
def is_palindrome(self): # palindrome doesn't apply # for empty linked lists
if not self.head:
return False
...
while start != end and start.prev != end: # return False if data don't match
if start.data != end.data:
return False # move forward from head # and backward from tail
start = start.next
end = end.prev # repeat until end is # no longer after start
if(start == end or start == end.next):
return True
