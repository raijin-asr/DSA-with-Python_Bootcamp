...
```
class Node:
    def **init**(self, data): # initialize a node with # data and next pointer
        self.data = data
        self.next = None
...
3. Start from the 'tail' node.

```

current = self.tail 2. Loop until the node is 'None' by utilizing the 'prev' pointer.
...
def traverse(self): # start at head
    current = self.head
    print("None", end = " <-> ") # loop until the node is None
    while current is not None:
...
def reverse_traverse(self): # start at tail
    current = self.tail
    print("None", end = " <-> ") # loop until node is None
    while current is not None:
        print(f"{current.data} <-> ", end="")
        current = current.prev
        print("None")
